...
def my_func(gpu_id):
    ...
    for _ in range(10):
        sleep(0.5)
        a = np.random.rand(1 * 4096 * 4096)
        if gpu_id is not None:
            import torch
            b = torch.tensor(a, device=f"cuda:{gpu_id}")
            b = torch.sqrt(b) * torch.exp(b)
        np.save("tmp.npy", a)
...
if __name__ == "__main__":
    # if this system is CUDA device enabled, set the following to True
    use_gpu = False
    ...
    setup_root_resource_logger(output_file="my_func_resource.log", interval=0.1, gpu_ids=[0] if use_gpu else None)
    ...
    my_func(0 if use_gpu else None)
